 ...
 * Visibility is controlled by the parent via <AnimatePresence>.
 * - Reveals "JAY" then "ALMINSHAWI" with character stagger + blur-in
 * - Animated progress bar to 100%, then signals parent via onProgressComplete
 * - The parent removes us; framer-motion plays the swipe-up exit before unmount
 */
export const LoadingScreen = ({ minDuration = 2200, onProgressComplete }) => {
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      ...
      if (pct < 100) {
        ...
        setTimeout(() => onProgressComplete?.(), 420);
      }
    };
    raf = requestAnimationFrame(tick);
    return () => cancelAnimationFrame(raf);
  }, [minDuration, onProgressComplete]);
  ...
  return (
    <motion.div
      ...
      exit={{
        y: "-100%",
        transition: { duration: 1.15, ease: [0.86, 0, 0.07, 1] },
      }}
      className="fixed inset-0 z-[100] bg-[#050505] flex flex-col"
    >
      {/* Content wrapper — fades & lifts slightly on exit for added depth */}
      <motion.div
      ...
      {/* Bottom-edge accent line that stays visible during swipe-up */}
